...
import type { MetricsDaemon } from "./index.js";
		...
		try {
			...
			if (process.platform === "linux") {
				...
				try {
					...
					const load = Number.parseFloat(output.trim());
			...
			if (process.platform === "linux") {
				try {
					...
					for (const line of diskstats.split("\n")) {
						...
						if (fields.length >= 4) {
							const reads = Number.parseInt(fields[3], 10) || 0;
							totalOps += reads;

							if (fields.length >= 8) {
								const writes = Number.parseInt(fields[7], 10) || 0;
								totalOps += writes;
							}
						}
					}
				...
				try {
					...
					for (const line of vmstat.split("\n")) {
						if (line.startsWith("pswpin ")) {
							const parts = line.split(/\s+/);
							if (parts.length >= 2) {
								swapIn = Number.parseInt(parts[1], 10) || 0;
							}
						} else if (line.startsWith("pswpout ")) {
							const parts = line.split(/\s+/);
							if (parts.length >= 2) {
								swapOut = Number.parseInt(parts[1], 10) || 0;
							}
						}
					}
				} catch (error) {
